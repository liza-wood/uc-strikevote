...
The Union's proposal of $54,000 for 50% sounds high, and it is. But my first questions are about trying to understand how high this really is, relative to current student salaries, and how this compares to other graduate student salaries.  
...
Want to dig into these numbrs yourself? All data and analyses can be found here: https://github.com/liza-wood/uc-strikevote  
...
```{r}
DT::datatable(uni_comp, rownames = F)
```
...
boxPlot_dollars_function <- function(fte50, fte100, title){
  ...
  stat_identity(geom = "text", colour = "white", size = 4,
                aes(label = text),
...
boxPlot_perc_function(grad50FTE_to_living_wage, grad100FTE_to_living_wage, 
                 "Current ncome as a proportion of living wage*")
```
...
```{r}
DT::datatable(uc_campus_wageDT, rownames = F)
```
